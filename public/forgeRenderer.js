...
function dessinerFond(ctx){ ctx.fillStyle = "#ffffff"; ctx.fillRect(0, 0, W, H); }
...
function dessinerPrimitive(ctx, e){
    ctx.beginPath();
    switch(e.forme){
        case "rectangle": ctx.rect(-e.taille/2, -e.taille, e.taille, e.taille*2); break;
        case "carre": ctx.rect(-e.taille/2, -e.taille/2, e.taille, e.taille); break;
        case "cercle": ctx.arc(0, 0, e.taille/2, 0, Math.PI*2); break;
        case "triangle": ctx.moveTo(0, -e.taille); ctx.lineTo(e.taille, e.taille); ctx.lineTo(-e.taille, e.taille); ctx.closePath(); break;
        case "losange": ctx.moveTo(0, -e.taille); ctx.lineTo(e.taille, 0); ctx.lineTo(0, e.taille); ctx.lineTo(-e.taille, 0); ctx.closePath(); break;
        case "croix": ctx.moveTo(-e.taille, 0); ctx.lineTo(e.taille, 0); ctx.moveTo(0, -e.taille); ctx.lineTo(0, e.taille); break;
        case "barre_verticale": ctx.rect(-e.taille*0.18, -e.taille, e.taille*0.36, e.taille*2); break;
    }
}
...
function dessinerElement(ctx, glyphe, element){
    ...
    const primitive = { forme: element.forme, taille: element.taille * glyphe.taille / 14 };
    dessinerPrimitive(ctx, primitive);
    element.plein ? ctx.fill() : ctx.stroke();
    ctx.restore();
}

function dessinerGlyphe(ctx, glyphe, angle){
    const rayon = glyphe.rayon + glyphe.offset;
    const x = C + Math.cos(angle) * rayon;
    const y = C + Math.sin(angle) * rayon;
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(angle + glyphe.rotation * Math.PI / 180);
    if(glyphe.miroir) ctx.scale(-1, 1);
    ctx.strokeStyle = couleur(glyphe.couleur);
    ctx.fillStyle = couleur(glyphe.couleur);
    ctx.lineWidth = glyphe.epaisseur;
    glyphe.elements.forEach(element => dessinerElement(ctx, glyphe, element));
    ctx.restore();
}
